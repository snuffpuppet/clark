...
| `extraction-rules.md` | Rules R1 to R25 and the grading conditions, with test passages. Carries its own version. |
| `IMPLEMENTATION-PLAN.md` | The build plan, with the formats the scripts agree on. |
| `process-audit-T001-T002.md` | The audit of every techm-bss process against its transcripts that led to stage S4 and rules R20 to R25 (ingester 0.10.0). The evidence for those rules and the draft of the first flow reference. |
...
| `runbooks/` | `S0.md` to `S4.md`, `staged-review.md`, `evaluate.md`, `summary.md`. The source of truth for what each stage reads, produces, checks and reports; the skill file operates them and does not restate them. |
...
| S3 Write | `bin/s3-write` | Both signed files | Registers, record, ledger, stakeholder edits, History lines on changed items, session log; VTT moved to `processed/` | Written |
| S4 Processes | The skill, then the human, then `bin/s4-write` | Whole utterance table, signed sheet, the register as S3 left it | `Tnnn.process-evidence.md` (working), `Tnnn.processes.md` (review), `Tnnn.walkthrough-agenda.md`; then process files, claim mutations, open items, `index/walkthrough-agenda.md` | Processes file signed, every Flow verdict and item verdict filled |
| Summary | `bin/summary`, after every S3 | The written transcript's files and the Claude Code session files | `evaluation/Tnnn-summary.md`: model, time, tokens, share of the plan's five-hour and weekly limits, output, verdict outcomes, comparison with the previous transcript; `evaluation/ingestions.md` rebuilt with a column per transcript | None |
...
- **F6 Claims.** A system or process file holds its claims as sections `## f1 <description>` (system facts), `## s1 <description>` (process steps) or `## n1 <description>` (process facts), each with `- key: value` lines (kind, performed-by, system, follows, when, hands-to, status, replaced-by, confidence, asserted-by, episode, session, evidence list). A step carries `follows` (the steps before it as `sN`, joined by `; `, blank for the first), `when` (its variant or condition) and `hands-to` (the process it passes to); a process fact carries `kind` (Rule, Volume, Timing, Pain point) and may carry `follows` for the step it qualifies. A Withdrawn claim carries `replaced-by`. Order in a process is held by `follows`, never by section number. Claim ids are `SYS-0002.f1`, `PRC-0006.s5`, `PRC-0006.n1`. Sections are never renumbered or removed. A process file's frontmatter adds `outcome`, `upstream`, `downstream` and, when Withdrawn, `replaced-by` (design 4.1).
...
- **F11 Ingestion summary.** `evaluation/Tnnn-summary.md` from `templates/ingestion-summary.md`: header fields, then Transcript, Time, Tokens, Cost, Plan usage (from the status line samples in `~/.claude/usage-samples.tsv`), S4 processes (measured on its own window once S4 is written, with `s4_` metrics), Output, Accuracy, Compared with the previous transcript, Caveats, and a last section `## Metrics` of `- key: value` lines that the next transcript's summary reads for its comparison. Keys are fixed by `bin/summary`; a key missing from an older summary is shown as not recorded.
- **F12 Ingestion comparison.** `evaluation/ingestions.md`, one per engagement: a markdown table with one row per metric and one column per transcript, read from the Metrics section of every `evaluation/Tnnn-summary.md` in transcript order, plus a column for the change between the two most recent. Rebuilt by `bin/summary` after every ingestion; never edited.
- **F13 Processes file.** `sessions/Tnnn/Tnnn.processes.md` from `templates/processes.md`, the review file of stage S4. Header as a dossier, with `- Completed by:` and `- Completed on:`. One section per process, `## Tnnn-Pnn <title>`, carrying `- Process:` (`new` or the PRC id), `- Flow verdict:` (blank, Accept, Edit or Reject) and `- Bulk accept:`, then a `### Flow` table (`| Step | Actor | Action | System | Follows | When | Hands to | Item |`) in flow order, then F4 item blocks in flow order. Kinds in a processes file: PRC, PRC.step, PRC.fact, Question, OI, and claim mutations (a PRC.step, PRC.fact or SYS.fact block whose Target is an existing claim id). A step's `- Follows:` names earlier items (`item 05`) or existing steps (`s3`, `PRC-0006.s3`); a `- Replaced by:` names items or ids. A section whose Flow verdict is Reject writes nothing. Bulk accept counts only once the Flow verdict is Accept or Edit. The file is signed when Completed by and Completed on are set and every Flow verdict and item verdict is filled (`processes_signed` in `bin/lib.sh`).
- **F14 Walkthrough agenda.** `sessions/Tnnn/Tnnn.walkthrough-agenda.md` from `templates/walkthrough-agenda.md`: per process, the flow so far and every gap, contradiction and Second-hand step, grouped under the role who should answer. `index/walkthrough-agenda.md` is the rollup of open process questions across the engagement, generated by `render-index` from the open items that carry `clarifies PRC-...`.
...
| `stage` | `stage <engagement> <Tnnn> <status\|tasks\|S0\|S1\|S3\|S4\|check\|score\|summary>` | Driver. Reports the state, runs a stage, refuses a stage whose predecessor is unsigned, writes a run log. |
