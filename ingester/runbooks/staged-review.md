...
- Stop 2: `Tnnn.dossier.md` containing only current-state claims (SYS, SYS.fact) and Questions. Processes are not in the dossier; runbook S4 builds and reviews them after S3. The reviewer gives verdicts on those and writes `- Stage 2 confirmed by: <name>, <date>` in the header.
