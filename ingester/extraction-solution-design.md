...
Ingestion is a Claude Code session started inside the engagement folder, `engagements/<name>/`, which is how the engagement is known (1.6). The user invokes the ingest skill with the transcript file and, optionally, the meeting subject, for example `/ingest-transcript ~/Downloads/TechnicalSyncUp.vtt "entity upgrade for multi-gig orders"`. The subject is recorded on the session sheet and the transcript register and guides episode and topic reading (5.3) without restricting it.
