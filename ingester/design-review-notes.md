...
| 10 September 2026, engagement files | 7.1 | Each engagement carries its own `CLAUDE.md` (how to start a session here and reach the ingester) and `LOG.md` (running log appended by the scripts and by hand), at Adam's direction. This relaxes "an engagement contains nothing that describes how ingestion works" to a pointer and a log. |
